/**
 * Deterministic differential corpus (Phase 1.3). Pure: every input is a
...
function* derives(): Generator<Recipe> {
  for (const km of [
    ...
    for (const type of ["x25519", "ec", "ed25519", "sr25519"] as const)
      yield { k: "derive", type, km };
}
...
function* signings(): Generator<Recipe> {
  ...
  for (const scheme of ["schnorr", "ecdsa", "ed25519", "sr25519"] as Scheme[]) {
    for (const key of keys) {
      ...
      for (const message of [WOLF, cyc(0), cyc(100, 7)]) {
        ...
        } else if (scheme === "sr25519") {
          yield { k: "sign", scheme, key, message }; // randomised; verify-only
...
function* pkbs(): Generator<Recipe> {
  for (const seed of [RUST_SEED, cyc(32, 1), cyc(16, 0x99), cyc(64, 0x21)])
    yield { k: "pkb", seed };
  for (const sigScheme of ["schnorr", "ecdsa", "ed25519", "sr25519"] as Scheme[])
...
function* decodes(): Generator<Recipe> {
  ...
  const cases: [DecodeType, string][] = [
    ...
    ["signingPriv", "d99c55" + bstr32],
    ["signingPriv", "d99c55" + "82" + "02" + bstr32],
    ["signingPub", "d99c56" + bstr32],
    ["signingPub", "d99c56" + "82" + "01" + "5821" + "02" + "00".repeat(32)],
    ...
    ["privateKeys", "d99c4d" + "82" + "d99c55" + bstr32 + "d99c4a" + bstr32],
