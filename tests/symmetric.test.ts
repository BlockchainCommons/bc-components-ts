...
describe("SymmetricKey", () => {
  ...
  describe("accessors", () => {
    ...
    it("should return string representation", () => {
      ...
      // the reference's `Display`: the short reference (digest of the tagged CBOR), not the key bytes
      expect(key.toString()).toBe(`SymmetricKey(${key.refHexShort()})`);
      expect(key.refHexShort()).toMatch(/^[0-9a-f]{8}$/);
      expect(key.toString()).not.toContain("80818283");
    });
  });
  ...
  describe("CBOR serialization", () => {
    ...
    it("should roundtrip through untagged CBOR", () => {
      ...
      const restored = SymmetricKey.codec.decodeUntagged(decodeCbor(data));
...
describe("EncryptedMessage", () => {
  ...
  describe("RFC test vector", () => {
    it("should encrypt to expected ciphertext", () => {
      ...
      const encrypted = key.encrypt(PLAINTEXT, { aad: AAD, nonce: nonce });
  ...
  describe("accessors", () => {
    ...
    it("should return AAD", () => {
      const key = SymmetricKey.random();
      const encrypted = key.encrypt(PLAINTEXT, { aad: AAD });
  ...
  describe("equality", () => {
    ...
    it("should be equal to another message with same components", () => {
      const key = SymmetricKey.fromHex(KEY_HEX);
      const nonce = Nonce.fromHex(NONCE_HEX);
      const encrypted1 = key.encrypt(PLAINTEXT, { aad: AAD, nonce: nonce });
      const encrypted2 = key.encrypt(PLAINTEXT, { aad: AAD, nonce: nonce });
    ...
    it("should not be equal with different nonce", () => {
      const key = SymmetricKey.random();
      const encrypted1 = key.encrypt(PLAINTEXT, { aad: AAD });
      const encrypted2 = key.encrypt(PLAINTEXT, { aad: AAD });
  ...
  describe("CBOR serialization", () => {
    ...
    it("should roundtrip through untagged CBOR", () => {
      ...
      const restored = EncryptedMessage.codec.decodeUntagged(decodeCbor(data));
