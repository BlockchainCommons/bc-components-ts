...
fn code_of_debug(d: &str) -> String {
    ...
    // The reference wraps dcbor failures as `Error::Cbor`; a raw dcbor variant name (from a
    // `TryFrom<CBOR>` that does not wrap) is the same class. The port reports both as `Cbor`
    // since follow-up Phase 2 (the boundary rule).
    match name.as_str() { "Cbor" | "WrongType" | "WrongTag" | "NonCanonical" | "Underrun" | "UnexpectedBreak" | "UnexpectedTag" | "Custom" | "NonCanonicalNumeric" => "Cbor".into(), _ => name }
}
...
fn signing_priv(scheme: &str, key: &[u8]) -> Result<SigningPrivateKey> {
    Ok(match scheme {
        ...
        _ => return Err(Error::general("unknown signature scheme")),
    })
}
...
fn run(r: &J) -> String {
    ...
    match k.as_str() {
        ...
        "seal" => {
            ...
            let pt = bytes(&r["plaintext"]);
            let aad: Option<Vec<u8>> = r.get("aad").map(bytes);
            let nonce = match r.get("nonce") { Some(n) => Some(tryc!(Nonce::from_data_ref(bytes(n)))), None => None };
            let sealed = if aad.is_none() && nonce.is_none() { SealedMessage::new(&pt, &pubk) } else { SealedMessage::new_opt(&pt, &pubk, aad, nonce) };
        ...
        // ---- follow-up Phase 1.1 (V1/V2) ----
        "verifyStrict" => {
            let pubk = SigningPublicKey::Ed25519(tryc!(Ed25519PublicKey::from_data_ref(bytes(&r["pub"]))));
            let sig = tryc!(Signature::ed25519_from_data_ref(bytes(&r["sig"])));
            let msg = bytes(&r["message"]);
            if pubk.verify(&sig, &msg) { "valid".into() } else { "invalid".into() }
        }
        "decodeUntagged" => {
            // The reference has one door, `from_tagged_cbor_data`; it has no untagged fallback (B2).
            let data = hex::decode(s(r, "hex")).unwrap();
            macro_rules! decu { ($t:ty) => { { let v = tryc!(<$t>::from_tagged_cbor_data(&data)); cod(&v) } } }
            match s(r, "type").as_str() {
                "digest" => decu!(Digest), "nonce" => decu!(Nonce), "salt" => decu!(Salt), "arid" => decu!(ARID), "xid" => decu!(XID),
                "reference" => decu!(Reference), "symmetricKey" => decu!(SymmetricKey), "uuid" => decu!(UUID),
                "x25519Priv" => decu!(X25519PrivateKey), "x25519Pub" => decu!(X25519PublicKey), "privateKeyBase" => decu!(PrivateKeyBase),
                "seed" => decu!(Seed), "signature" => decu!(Signature), "signingPriv" => decu!(SigningPrivateKey), "encryptedMessage" => decu!(EncryptedMessage),
                _ => "js-only".into(),
            }
        }
        "signingDefault" => {
            // `SignatureScheme::default()` is what `keypair_using` picks; the reference has no `SigningPrivateKey::random`.
            let mut g = rng_of(&r["rng"]);
            let (p, _) = tryc!(keypair_using(&mut g));
        ...
        "kdfDomain" => {
            // Anything that is not an integer in the field's width cannot reach the reference: js-only.
            ...
            match s(r, "method").as_str() {
                ...
                _ => match (int("logN", u8::MAX as u64), int("r", u32::MAX as u64), int("p", u32::MAX as u64)) {
        ...
        "summary" => {
            // The reference's registry (registered in `main`), applied the way dcbor's
            // diagnostic printer applies it: to the untagged content, non-flat.
            let data = hex::decode(s(r, "hex")).unwrap();
            let cbor = match CBOR::try_from_data(&data) { Ok(c) => c, Err(_) => return "throw:Cbor".into() };
            let (tag, content) = match cbor.try_into_tagged_value() { Ok(x) => x, Err(_) => return "untagged".into() };
            // Clone the summarizer out of the store first: the global store is a lock, and a
            // summarizer that prints a tag name (`to_string()` of a key) takes it again.
            let f = dcbor::with_tags!(|t: &TagsStore| t.summarizer(tag.value()).cloned());
            match f.map(|f| f(content, false)) { None => "none".into(), Some(Ok(v)) => v, Some(Err(_)) => "error".into() }
        }
        _ => "js-only".into(),
    }
}

/// E1 made explicit (follow-up Phase 1.2): every "both reject, different
/// code" pair the corpus produces, as (kind, reference code, TypeScript code).
/// A pair outside this table is a MISMATCH.
const E1: &[(&str, &str, &str)] = &[
    ("value", "throw:Error", "throw:Sskr"),                  // SSKRShare::from_data: bc-sskr's error surfaces as a bare `Error`
    ("value", "throw:panic", "throw:InvalidData"),           // ECPublicKey::from_data_ref panics (not Err) on a non-point
    ("urParse", "throw:Cbor", "throw:UnexpectedType"),      // UR of another type
    ("urParse", "throw:Cbor", "throw:Bytewords"),           // unparsable bytewords body
    ("decode", "throw:Cbor", "throw:InvalidData"),          // wrong shape inside the tag
    ("decode", "throw:Cbor", "throw:InvalidSize"),          // wrong length inside the tag
    ("decode", "throw:Cbor", "throw:PostQuantum"),          // wrong ML-KEM/ML-DSA level or empty key
];
/// D6 (follow-up Phase 2, Q4): the port rejects a zero secp256k1 scalar or an off-curve
...
fn pending(_k: &str, _got: &str, _want: &str) -> Option<&'static str> {
    None
}

/// D1: the reference's `Display` strings for KDF params are not the
/// TypeScript ones; compare structurally. D2: JS-only surfaces (ML-DSA/ML-KEM key generation, SSH-agent params, SSH ECDSA key
/// generation from a seed, structural ML-KEM sealing).
fn expected_divergence(r: &J, got: &str, want: &str) -> Option<&'static str> {
    if got == "throw:General" && want.starts_with("unsupported:") { return Some("D2"); }
    ...
    // D2: seeded ML-KEM key generation exists here (follow-up W2 threads `rng` through
    // `createEncapsulationKeypair`); the reference's `keypair_opt_using` refuses it.
    ...
    if (k == "value" || k == "decode") && D5_TYPES.contains(&ty) {
...
fn main() {
    ...
    for v in &file.vectors {
        ...
        // The reference cannot express the input or has no such operation (the plan's §3 / D2).
        if got == "js-only" { js_only += 1; continue; }
        if let Some(id) = expected_divergence(&v.recipe, &got, &v.expect) { expected += 1; *tally.entry(id).or_default() += 1; eprintln!("expected-divergence [{id}] {} | rust={} | ts={}", v.name, &got.chars().take(60).collect::<String>(), &v.expect.chars().take(60).collect::<String>()); continue; }
        mismatch += 1;
    ...
    let breakdown: Vec<String> = tally.iter().map(|(id, n)| format!("{id} {n}")).collect();
    println!("{} vectors - {ok} match, {expected} expected-divergence ({}), {js_only} js-only, {mismatch} MISMATCH", file.vectors.len(), breakdown.join(", "));
