...
export class SealedMessage implements ToCbor, ToUR {
  ...
  static seal(
    ...
    { aad = new Uint8Array(0), nonce, rng }: { aad?: Uint8Array; nonce?: Nonce } & RngOptions = {},
  ): SealedMessage {
    const opts = rng === undefined ? {} : { rng };
    const [sharedSecret, ciphertext] = recipient.encapsulateNewSharedSecret(opts);
    const encryptedMessage = sharedSecret.encrypt(plaintext, {
      aad,
      nonce: nonce ?? Nonce.random(opts),
    });
